...
def igprocess(pics, _threads)
  ...
  pics.sort.each do |original|
    ...
    next unless thread_count >= $instagram_threads
...
def igify(original, dir = dirslash($current_set + $instagram_image_directory), _threads = $instagram_threads)
  FileUtils.mkdir_p(dir) unless $dir_checked == true && Dir.exist?(dir) == true
  $dir_checked = true
  destination = dir + $instagram_suffix + original.split('/').last.to_s

  if File.exist?(destination) && $instagram_skip_if_exists == 'yes'
...
def igactual(original, destination)
  ...
  shade = if brightness.to_i > 32_768
            'white'
          else
            'black'
          end
