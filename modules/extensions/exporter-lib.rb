...
require 'mini_magick' if $export_and_resize == 'yes'

$global_menu << 'export|EXPORT to maintained image repo'

require 'mini_exiftool' if $try_and_group_set_by_changing_exif == 'yes'
...
def doexport
  chkmk($export_to)
  dirs = []
  dirs << $final_image_directory if $export_final_image_directory == 'yes'
  unless $export_extra_directories.empty?
