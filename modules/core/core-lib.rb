...
def post_mv(source, destination)
  if $during_post_actions_overwrite_if_file_already_exists == 'no'
    if File.exist?(destination + filename(source))
...
def move_pics_to_next(favor = '')
  ...
  if pics.empty?
    ...
    if favor.empty?
      ...
      pics.each do |pic|
        masters << pic if pic.include?(favor)
      end
      # This is not optimal!
      masters.each do |master|
        master_filename = filename(master)
        pics.each do |pic|
          discards << pic if pic.include?(master_filename.gsub(favor, ''))
        end
      end
