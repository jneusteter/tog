...
if $menu_select == 'help'
  ...
  allmods['extensions'].sort.each do |mod|
    next unless allmods['installed'].include?(mod)
...
eval($menu_select) if $menu_select && $menu_select[0..3] == 'post'
...
if $menu_select == 'find'
  ...
  matches = []
  allsets.each do |set|
    matches << set if set.include? searchstring
  end
