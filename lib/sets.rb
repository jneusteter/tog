...
def save_current_set(path, display = 'no')
  ...
  $current_set = path
  ln_current_set
  banner_current_set unless display == 'no'
end

def check_set
  if Dir.exist?($current_set + dirslash($final_image_directory)) == false
    banner_current_set
...
def load_current_set(display = 'no')
  if $current_set_file
    if File.exist?($current_set_file)
      current_set = File.read($current_set_file)
      if !Dir.exist?(current_set)
        return false
      else
        $current_set = current_set
        banner_current_set if display == 'yes'
        check_set
        return current_set
      end
     end
...
def allsets
  ...
  $set_roots.each do |root|
    ...
    dirs += Dir.glob(root.split('|').last).map(&File.method(:realpath)).select { |f| File.directory? f }
  end
  dirs
end
